...
bool PN532::readPassiveTargetID(uint8_t cardbaudrate, uint8_t *uid, uint8_t *uidLength, uint16_t timeout)
{
    ...
    int16_t res = HAL(readResponse)(pn532_packetbuffer, sizeof(pn532_packetbuffer), timeout);

    // read data packet
    if (res < 0) {
        Serial.println("read data packet failed");
        Serial.print("Res: ");
        Serial.println(res);
        return 0x0;
    }
    ...
    */
    ...
    if (pn532_packetbuffer[0] != 1) {
        Serial.println("packetBuffer[0] != 1");
        return 0;
    }
